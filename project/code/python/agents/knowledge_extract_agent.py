...
class KnowledgeExtractAgent:
    ...
    def __init__(self) -> None:
        self.llm = build_chat_openai(component="knowledge_extract")
    ...
    async def extract(self, chunks: list[DocumentChunk]) -> list[ExtractionResult]:
        ...
        return self.resolve_entities(merged)
    ...
    def _parse_response(self, raw: str, source_id: str) -> ExtractionResult:
        try:
            ...
            logging.getLogger(__name__).warning(
                "Knowledge extraction JSON parse failed for %s; raw_head=%r",
                source_id,
                (raw or "")[:200],
            )
    ...
    def resolve_entities(results: list[ExtractionResult]) -> list[ExtractionResult]:
        """别名 + 相似度对齐：将「腾讯公司」等合并到规范名，并改写关系两端。"""
        from services.entity_align import build_canonical_map
        ...
        for result in results:
            ...
            for rel in result.relations:
                if rel.head:
                    names.append(rel.head)
                if rel.tail:
                    names.append(rel.tail)
            for ev in result.events:
                names.extend([p for p in (ev.participants or []) if p])

        if not names:
            return results

        canon = build_canonical_map(names)
        aligned: list[ExtractionResult] = []
        for result in results:
            ents: list[Entity] = []
            seen: set[str] = set()
            for ent in result.entities:
                name = canon.get(ent.name, ent.name)
                key = f"{name}::{ent.type}"
                if key in seen:
                    continue
                seen.add(key)
                ents.append(Entity(name=name, type=ent.type, description=ent.description))
            rels = [
                Relation(
                    head=canon.get(rel.head, rel.head),
                    relation=rel.relation,
                    tail=canon.get(rel.tail, rel.tail),
            ...
            uniq_rels: list[Relation] = []
            for rel in rels:
                k = (rel.head, rel.relation, rel.tail)
                if k in rel_seen:
                    continue
                rel_seen.add(k)
            ...
            events = [
                KnowledgeEvent(
                    trigger=ev.trigger,
                    type=ev.type,
                    participants=[canon.get(p, p) for p in (ev.participants or [])],
                )
                for ev in result.events
            ]
            aligned.append(
